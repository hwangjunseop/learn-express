...
const dotenv = require('dotenv'); // dotenv 라이브러리 -> 환경 변수 처리
...
app.set('port', process.env.PORT || 9000); // 앱의 포트를 성정 -> 환경 변수에 포트가 지정되어있지 않으면 3000번 포트를 사용
...
app.use(cookieParser(process.env.COOKIE_SECRET));
    // COOKIE_SECRET -> 제공한 비밀키를 통해 해당 쿠키가 내 서버에 만든 쿠키임을 검증
...
const upload = multer({
    ...
    storage: multer.diskStorage({
        destination(req, file, done) {
            // 파일의 확장자 추출
            const ext = path.extname(file.originalnme);
            // 파일명에 현재 시간을 더하여 중복을 피함
            done(null, path.basename(file.originalnme, ext) + Date.now() + ext);
        },
    }),
    // 업로드될 파일의 최대 크기 제한 -> 5MB(5 * 1024 * 1204byte)로 설정
...
app.get('/',(req, res, next) => {
    ...
    // res -> 응답에 대한 정보가 들어있는 겍체
    // next -> 다음 미들웨어로 넘어가는 함수, next를 실행하지 않으면 다음 미들웨어가 실행되지 않음.
    // (req, res) => {} -> 다음으로 실행될 콜백 함수를 정의, 이 함수는 req, res를 인자로 받음.
    // throw net Error -> 에러를 발생시키고, 이를 등로고딘 에러 처리 미들웨어로 전달

// 에러처리 미들웨어 -> 매개 변수가 err, req, res, next로 4개 -> 반드시 매개변수가 4여야 함.
// 기본적으로 익스프레스가 에러를 처리하긴 하지만, 직접 에러 처리 미들웨어를 연결해주는 것을 권장
// 에러처리 미들웨어는 특별한 경우기 아니면 가장 아래에 위치함.
...
// 앱이 설정한 포트에서 대기상태로 시작됩니다.
// 서버가 시작되면 해당 포트에서 대기중 메시지를 로그에 출력합니다.
